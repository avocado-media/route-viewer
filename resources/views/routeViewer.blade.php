...
<div class="mx-auto container">
    <section x-data="loadRoutes">
        <div class="flex justify-between mb-4">
            ...
            <div
                class="relative max-w-xs w-full flex items-center ring-1 rounded-lg ring-slate-700 focus-within:ring-slate-500 transition-colors">
                <input type="text" name="search" placeholder="Search..."
                       class="block w-full rounded-md bg-transparent border-0 pr-12 shadow-sm focus:ring-0 sm:text-sm peer"
                       x-ref="inputSearch" x-model="search" @keydown.window.prevent.cmd.k="$refs.inputSearch.focus()">
                <kbd
                    class="h-6 top-1/2 mr-1.5 -translate-y-1/2 absolute inset-y-0 right-0 flex py-1.5 items-center rounded border border-slate-700 px-2 font-sans text-sm font-medium text-slate-700 peer-focus:text-slate-500 peer-focus:border-slate-500 transition-colors">⌘K</kbd>
            </div>
        </div>
                            ...
                            <td class="whitespace-nowrap py-4 pl-4 pr-3 sm:pl-6">
                                <ul class="flex">
                                    <template x-for="method in route.methods">
                                        <li x-text="method" class="badge mr-2 last:mr-0"
                                            :class="'badge--' + method.toLowerCase()"></li>
                                    </template>
                                </ul>
                            </td>
                            <td class="whitespace-nowrap px-3 py-4" x-text="route.uri"/>
                            <td class="whitespace-nowrap px-3 py-4" x-text="route.name"/>
                            <td class="whitespace-nowrap px-3 py-4 font-mono" x-text="route.action"/>
                            <td class="whitespace-nowrap py-4 pl-3 pr-4">
                                <ul class="flex">
                                    <template x-for="method in route.middleware">
                                        <li x-text="method"
                                            class="mr-1 last:mr-0 after:content-[','] last:after:content-['']"></li>
                                    </template>
                                </ul>
                            </td>
                        </tr>
